...
public class GadisFinder extends AbstractFinder implements Finder {
  ...
  private void getSession() {
    ...
    if (diff > 30 || this.iSessionId == null) {
      ...
      try {
        ...
        final BodyPublisher body = HttpRequest.BodyPublishers.ofString("resource=postalCode&cl_lang=es&cl_postal_code=15001");
   ...
   */
  protected Builder addHeaders(final Builder requetsBuilder) {
    this.getSession();
    requetsBuilder.setHeader("Content-Type", "application/x-www-form-urlencoded; charset=UTF-8");
    requetsBuilder.setHeader("Cookie", this.iSessionId
    ...
    //requetsBuilder.setHeader("Cookie","PHPSESSID=9ll7micjao18c94bo1aff582oj; 
    // melindres_options={%22required%22:true%2C%22analytics%22:true}; _ga_FN6H4R59BJ=GS1.1.1694968677.1.1.1694968710.0.0.0; _ga=GA1.1
    // .1593199813.1694968678");
  ...
  protected List<Product> getProductList(final JsonObject responseJsonObj) {
    ...
    final JsonArray productsJsonList = responseJsonObj.
        get("dato").getAsJsonObject().get("productos").getAsJsonArray();
    ...
    if (productsJsonList != null) {
      for (final JsonElement productJson : productsJsonList) {
        ...
        if (productObj.get("precioUnidad") != null && productObj.get("medidaTxt") != null && StringUtils.isNotBlank(
            productObj.get("medidaTxt").getAsString())) {
        ...
        final JsonArray imagesList = ((JsonArray) productObj.get("imagenes"));
